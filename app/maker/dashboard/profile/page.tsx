...
export default function MakerProfilePage() {
  ...
  const { currentUser, houseMakers, accounts, updateHouseMakerProfile } = useStore();
  ...
  const [orgName, setOrgName] = useState(houseMaker?.name ?? "");
  const [phone, setPhone] = useState(houseMaker?.phone ?? "");
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const result = updateHouseMakerProfile({
      orgName,
      phone,
      fullName,
      email,
  ...
  if (!houseMaker) return null;

  return (
    <div className="max-w-lg mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">
          {BUILDER_TYPE_LABEL[houseMaker.builder_type]}情報の編集
        </h1>
      ...
      <button onClick={() => router.push("/maker/dashboard")} className="btn-secondary w-full">
        ダッシュボードへ戻る
      </button>
    </div>
  );
