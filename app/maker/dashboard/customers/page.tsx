...
export default function MakerCustomersPage() {
  ...
  const myConnections = useMemo(
    () =>
      customerConnections
        .filter((c) => c.house_maker_id === houseMakerId)
        .map((c) => ({ ...c, customers: customers.find((cu) => cu.id === c.customer_id) }))
        .sort((a, b) => (a.created_at < b.created_at ? 1 : -1)),
    [customerConnections, houseMakerId, customers]
  );

  const counts = useMemo(() => {
    const c: Record<AffiliationStatus, number> = { pending: 0, approved: 0, rejected: 0 };
    myConnections.forEach((conn) => c[conn.house_maker_status]++);
    return c;
  }, [myConnections]);

  const filtered = useMemo(
    () => (filter === "all" ? myConnections : myConnections.filter((c) => c.house_maker_status === filter)),
        ...
        {filtered.map((c) => (
          <div key={c.id} className="card p-4 md:p-5 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <span
                className={`inline-flex items-center gap-1 mt-2 rounded-full border px-2.5 py-0.5 text-xs font-bold ${AFFILIATION_STATUS_COLOR[c.house_maker_status]}`}
              >
                {AFFILIATION_STATUS_LABEL[c.house_maker_status]}
              </span>
            </div>
            {c.house_maker_status === "pending" && (
              <div className="flex gap-2 shrink-0">
                <button
                  onClick={() => updateCustomerConnectionStatus(c.id, "house_maker", "approved")}
                ...
                <button
                  onClick={() => updateCustomerConnectionStatus(c.id, "house_maker", "rejected")}
