...
ログイン・新規登録・ステータス確認などの単体カードページには `.auth-bg` クラスで、部屋の中の
灯りをイメージした温かみのあるグラデーション + ぼかしブロブを背景に配置しています。トップページ
(`/`)は夕焼け空のグラデーションに、`components/HouseSkyline.tsx`（SVGの家並みシルエット）を
下部に敷いています。一覧のクリック可能な行には`.list-row`（ホバーで少し浮き上がる）、データが
0件の箇所には`.empty-state`（アイコン付き）を使っており、こちらも`app/globals.css`で一括調整
できます。

※ 実在する住宅の写真は使用していません。フリー素材サイトで見つかる住宅写真の多くは
Unsplash+など有料ライセンスのもので、無断で埋め込むとライセンス違反になり得るため、
CSSとSVGのみで「家」「部屋の灯り」を表現する方針にしています。また、このアプリの実行環境には
画像生成ツールがないため「AIが生成した家の写真」も作れません。代わりに `components/HouseIllustration.tsx`
に、家の外観を手描きしたSVGイラストを1つ作り、トップページの主役グラフィック（大きめ表示）と
各ログイン・新規登録ページ（アフター会社・ハウスメーカー/工務店・居住者様、計6画面）の
見出し上に小さく共通して配置しています。ライセンス確認済みの実写真をお持ちであれば、
`app/globals.css` の `.home-hero-bg`（トップページ）・`.auth-bg`（ログイン等）に
`background-image: url(...)` を1行追加するか、`HouseIllustration` の呼び出しを `<img>` タグに
差し替えるだけで、実写真に切り替えられます。

各ダッシュボードのトップには、ハウスメーカーのコーポレートサイトのような、グラデーションと
幾何学模様をあしらったヒーローバナー（`components/DashboardHero.tsx`）を配置しています。
バナー内にも `HouseSkyline` をうっすら白で重ね、家並みのモチーフで統一感を出しています。

管理画面全体（アフター会社・ハウスメーカー/工務店・居住者マイページ）の背景も `.dashboard-bg`
クラスで、データの読みやすさを保ちながらほんのり暖色を混ぜた色味にしています。居住者・工務店
向けの公開受付フォーム（`/request/[company_id]`）のような縦に長いページには `.warm-bg`
（装飾ブロブなしの固定グラデーションのみ）を使い、スクロールしても背景が破綻しないようにして
います。
